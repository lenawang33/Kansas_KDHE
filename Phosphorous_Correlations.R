...
Haney_Test <- Data %>% select(Crop.2023,
  ...
  rename(`H3A-Inorganic Phosphorous (mg/L)` = "H3A_IP",
         `H3A-Organic Phosphorous (mg/L)` = "H3A_OP",
         `Organic Matter (%)` = "OM.",
...
Haney_Test[,8:24] <- as.numeric(unlist(Haney_Test[,8:24]))

Long_H <-  pivot_longer(Haney_Test, cols = c(8:24), names_to = "Variable", values_to = "Value")
...
elist <- list("WEN-NO3 (mg/L)","pH","SoilHealth", "Organic Matter (%)", "WEOC (mg/L)","WEON (mg/L)", 
  ...
  for (e in 1:length(elist)){
    Properties <- Ranked %>% subset(Variable %in% c(elist[[e]])) %>%
      mutate(Properties_Rank = rank(Value),
             P_Rank = rank(`H3A-Inorganic Phosphorous (mg/L)`))
    Spearman <- cor.test(Properties$Properties_Rank, Properties$P_Rank, method = "spearman", exact = FALSE)
...
#write.csv(ModelDf, "H3A_P_Spearman_Correlations.csv")
...
WEN_Corplot <- ggplot(Corr) + 
  geom_point(aes(x = `Value`, y = `H3A-Inorganic Phosphorous (mg/L)`, color = Correlation, shape = `Cover & Till`, size = `P-value`)) + 
...
setwd("C:/Users/lwang03/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Kansas_Roar/Plots")
ggsave("H3A_IP_Correlations_No_Mean_Spearman.pdf", WEN_Corplot, height = 30, width = 30, limitsize = F)
...
for (s in 1:length(slist)){
  ...
  for (e in 1:length(elist)){
    Properties <- Ranked %>% subset(Variable %in% c(elist[[e]])) %>%
      mutate(Properties_Rank = rank(Value),
             P_Rank = rank(`H3A-Organic Phosphorous (mg/L)`))
    Spearman <- cor.test(Properties$Properties_Rank, Properties$P_Rank, method = "spearman", exact = FALSE)
    Df[e, "Field"] <- paste(slist[[s]])
    Df[e,"Variable"]<- paste(elist[[e]])
    Df[e, "Correlation"] <- Spearman[[4]]
...
WEN_Corplot <- ggplot(Corr) + 
  geom_point(aes(x = `Value`, y = `H3A-Organic Phosphorous (mg/L)`, color = Correlation, shape = `Cover & Till`, size = `P-value`)) + 
  guides(shape = guide_legend(override.aes = list(size = 10))) +
  facet_wrap(vars(Variable), scales = "free", strip.position = "bottom") +
  scale_color_gradient(low = "#DC3220", high = "#005AB5") + 
  scale_shape_manual(values = Tillage_Cover) + 
...
setwd("C:/Users/lwang03/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Kansas_Roar/Plots")
ggsave("H3A_OP_Correlations_No_Mean_Spearman.pdf", WEN_Corplot, height = 30, width = 30, limitsize = F)
